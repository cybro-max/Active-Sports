...
import { getPlayerIndexLite, isFullBuild, getPlayerIndexFull } from '@/lib/build-params';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Players — ${letter.toUpperCase()} | ActiveSports`,
    description: `Browse football players starting with ${letter.toUpperCase()}. View stats, profile, trophies and more.`,
    alternates: { canonical: `https://activesports.live/players/${letter}` },
  };
}

export default async function PlayersByLetterPage({ params }: Props) {
  const { letter } = await params;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 fade-up">
        <h1 className="text-3xl font-bold mb-1 text-[var(--primary)]">
          <span className="text-[var(--accent)]">Players</span> A–Z
        </h1>
        <p className="text-sm text-[var(--text-muted)]">
          {entries.length} player{entries.length !== 1 ? 's' : ''} starting with &ldquo;{letter.toUpperCase()}&rdquo;
        </p>
      </div>
      ...
      {entries.length === 0 ? (
        <div className="card p-12 text-center">
          <p className="text-[var(--text-muted)]">No players found starting with &ldquo;{letter.toUpperCase()}&rdquo;.</p>
        </div>
      ) : (
          ...
          {entries.map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[var(--text-body)] truncate group-hover:text-[var(--accent)] transition-colors">
                  {p.name}
                </p>
              </div>
